...
const BrandPanel = () => (
  <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-teal-600 via-teal-700 to-emerald-800 flex-col items-center justify-center p-12">
    ...
    <div className="relative z-10 text-white max-w-sm text-center">
      ...
      >
        Upload your financial statement and unlock intelligent insights about your finances.
      </motion.p>
...
function getPasswordStrength(pw: string) {
  if (!pw) return null;
  const checks = [pw.length >= 6, pw.length >= 10, /[A-Z]/.test(pw), /[0-9!@#$%^&*]/.test(pw)];
  const score = checks.filter(Boolean).length;
  if (score <= 1) return { score: 1, label: "Weak", color: "bg-red-400", textColor: "text-red-500" };
  if (score === 2) return { score: 2, label: "Fair", color: "bg-yellow-400", textColor: "text-yellow-600" };
...
const InputField = ({
  ...
  disabled, required, error, delay = 0, children,
}: InputFieldProps) => (
  <motion.div
    ...
    <div className="relative group">
      ...
      <input
        ...
        className={`block w-full h-12 pl-11 pr-4 rounded-xl border bg-gray-50/60 text-gray-900 placeholder-gray-400
          ...
          ${error ? "border-red-400 bg-red-50/40 focus:border-red-400 focus:ring-red-100" : "border-gray-200"}`}
      />
    </div>
    {error && (
      <motion.p
      ...
      >
        <span className="inline-flex w-3.5 h-3.5 rounded-full bg-red-100 border border-red-300 items-center justify-center text-[10px] font-bold text-red-600 flex-shrink-0">!</span>
        {error}
      </motion.p>
    )}
    {children}
        ...
        <div className="relative z-10 w-full max-w-md">
          ...
          >
            ...
            <form onSubmit={handleRegister} className="space-y-5" noValidate>
              ...
              >
                <FiShield className="w-3.5 h-3.5 text-teal-500 flex-shrink-0" />
